...
//    ];
    ...
    $scope.salvar = function (id) {
        var dados = $scope.campos;
        if ($scope.campos.id == null) {
            $http.post('http://localhost:8000/app_dev.php/rest/usuario/', dados)
            ...
            $http.put('http://localhost:8000/app_dev.php/rest/usuario/', dados)
                        .success(function(data, status){
                            $scope.campos.id = data.id;
    ...
    $scope.excluir = function (id) {
        ...
        $http.delete('http://localhost:8000/app_dev.php/rest/usuario/' + id.id)
                    .success(function(data, status){
                        alert('Usuário excluído com sucesso: ' + id.id + ' - ' + id.nome);
                        $http.get("http://localhost:8000/app_dev.php/rest/usuario")
                                            .success(function (response) {
                                                $scope.usuarios = response;
                                            });
